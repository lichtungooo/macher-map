...
export function ZoomButtons() {
  ...
  const zoom = (delta: number) => {
    ...
    const center = bounds.getCenter() // Exakter Mittelpunkt der sichtbaren Flaeche
    const target = Math.max(2, Math.min(19, map.getZoom() + delta))
    map.setView(center, target, { animate: true, duration: 0.4, easeLinearity: 0.25 })
  }
