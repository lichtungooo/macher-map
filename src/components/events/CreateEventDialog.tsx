  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(4px)' }}>
      <div className="relative w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-6 sm:p-8 shadow-xl max-h-[85vh] overflow-y-auto" style={{ background: '#fff', border: '1px solid rgba(10,10,10,0.06)' }}>
        ...
        <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '1.4rem', fontWeight: 400, color: '#0A0A0A', marginBottom: '1.2rem' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="z.B. Vollmond-Meditation am See" required className="w-full px-4 py-3 rounded-lg outline-none" style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {EVENT_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => setType(t.value)} className="px-3 py-1.5 rounded-full text-xs transition-all"
                  style={{ fontFamily: 'Inter, sans-serif', border: type === t.value ? '1px solid #D4A843' : '1px solid rgba(10,10,10,0.1)', color: type === t.value ? '#D4A843' : 'rgba(10,10,10,0.5)', background: type === t.value ? 'rgba(212,168,67,0.08)' : 'transparent', cursor: 'pointer' }}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={date} onChange={e => setDate(e.target.value)} required className="w-full px-3 py-3 rounded-lg outline-none" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Uhrzeit</label>
              <input type="time" value={time} onChange={e => setTime(e.target.value)} className="w-full px-3 py-3 rounded-lg outline-none" style={inputStyle} />
            </div>
          </div>

          <div>
            <label style={labelStyle}>Wiederholung</label>
            <select value={recurring} onChange={e => setRecurring(e.target.value)} className="w-full px-4 py-3 rounded-lg outline-none" style={inputStyle}>
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Beschreibe deine Absicht. Was wollt ihr gemeinsam tun?" rows={3}
