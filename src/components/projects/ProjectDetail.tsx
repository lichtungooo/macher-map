...
import { X, ExternalLink, Camera, Pencil, Trash2, Plus, Check, Video, Hash, Trees, Navigation } from 'lucide-react'
    ...
    return (
      <div className="fixed inset-0 z-[1500] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.3)' }}>
        <div className="rounded-xl px-5 py-3" style={{ background: '#fff' }}>
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: 'rgba(10,10,10,0.55)' }}>Laedt...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 z-[1500] flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(4px)' }}>
      <div className="relative w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-xl overflow-hidden" style={{ background: '#fff', border: '1px solid rgba(10,10,10,0.06)', maxHeight: '90vh' }}>
        ...
        {/* Bild als Header oder Platzhalter */}
        {project.image_path ? (
          ...
          <button onClick={() => fileRef.current?.click()}
            ...
            style={{ background: 'rgba(192,112,144,0.06)', border: 'none', cursor: 'pointer', borderBottom: '1px solid rgba(10,10,10,0.06)' }}>
            <Camera size={18} style={{ color: '#C07090' }} />
            <span style={{ ...font, fontSize: '0.72rem', fontWeight: 500, color: '#C07090' }}>Bild hinzufuegen</span>
          </button>
        ) : null}
        ...
        <button onClick={onClose}
          className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center"
          style={{ background: 'rgba(255,255,255,0.9)', border: 'none', cursor: 'pointer', zIndex: 10 }}>
          <X size={15} style={{ color: '#0A0A0A' }} />
        </button>

        <div className="overflow-y-auto px-5 py-5" style={{ maxHeight: project.image_path ? 'calc(90vh - 176px)' : 'calc(90vh - 140px)' }}>

          {/* Label */}
          <p style={{ ...font, fontSize: '0.6rem', fontWeight: 500, color: '#C07090', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 6 }}>
            Projekt
          </p>
            ...
            <>
                ...
                <div className="rounded-xl p-4 mb-4" style={{ background: '#FAFAF8' }}>
                  ...
                  <div className="w-full rounded-full overflow-hidden" style={{ height: 8, background: 'rgba(10,10,10,0.06)' }}>
                    <div style={{
                      width: `${progressPct}%`,
                      height: '100%',
                      background: progress >= 1 ? 'linear-gradient(90deg, #7BAE5E, #D4A843)' : 'linear-gradient(90deg, #C07090, #E0A0B5)',
              ...
              {project.opencollective_url && (
                <a href={project.opencollective_url} target="_blank" rel="noopener noreferrer"
                  ...
                  style={{ ...font, fontSize: '0.85rem', fontWeight: 500, color: '#fff', background: 'linear-gradient(90deg, #C07090, #E0A0B5)', border: 'none', textDecoration: 'none' }}>
              ...
              {project.video_url && (
                <a href={project.video_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <Video size={14} style={{ color: '#C07090' }} />
                  Video ansehen
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {tags.map((t: string) => (
                      <span key={t} className="flex items-center gap-0.5 px-2 py-1 rounded-full"
                        style={{ background: 'rgba(192,112,144,0.07)', ...font, fontSize: '0.68rem', fontWeight: 500, color: '#C07090' }}>
                        <Hash size={9} />{t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                  <p style={{ ...font, fontSize: '0.62rem', fontWeight: 600, color: 'rgba(10,10,10,0.4)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                    Meilensteine
                  </p>
                  {isOwner && (
                    <button onClick={() => setNewMilestoneOpen(true)}
                      className="flex items-center gap-1 px-2 py-1 rounded"
                      style={{ ...font, fontSize: '0.65rem', color: '#C07090', background: 'rgba(192,112,144,0.06)', border: '1px solid rgba(192,112,144,0.15)', cursor: 'pointer' }}>
                ...
                {newMilestoneOpen && isOwner && (
                  <div className="rounded-xl p-3 mb-2 space-y-2" style={{ background: 'rgba(192,112,144,0.04)', border: '1px solid rgba(192,112,144,0.15)' }}>
                    <input type="text" value={newMilestoneTitle} onChange={e => setNewMilestoneTitle(e.target.value)}
                      ...
                      style={{ ...font, fontSize: '0.8rem', border: '1px solid rgba(10,10,10,0.08)', background: '#fff' }} />
                    ...
                    <textarea value={newMilestoneDesc} onChange={e => setNewMilestoneDesc(e.target.value)}
                      ...
                      style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '0.85rem', border: '1px solid rgba(10,10,10,0.08)', background: '#fff' }} />
                    <div className="flex gap-2">
                      <button onClick={addMilestone}
                        className="flex-1 py-2 rounded-lg" style={{ ...font, fontSize: '0.75rem', fontWeight: 500, color: '#fff', background: '#C07090', border: 'none', cursor: 'pointer' }}>
                        Speichern
                      </button>
                      <button onClick={() => setNewMilestoneOpen(false)}
                        className="px-4 py-2 rounded-lg" style={{ ...font, fontSize: '0.75rem', color: 'rgba(10,10,10,0.5)', background: '#fff', border: '1px solid rgba(10,10,10,0.1)', cursor: 'pointer' }}>
                        Abbrechen
                      </button>
                    </div>
                  </div>
                ...
                {milestones.length === 0 ? (
                  <p style={{ ...font, fontSize: '0.75rem', color: 'rgba(10,10,10,0.35)', fontStyle: 'italic' }}>
                    Keine Meilensteine.
                  </p>
                ) : (
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={saveEdit}
                  className="flex-1 py-2.5 rounded-lg" style={{ ...font, fontSize: '0.82rem', fontWeight: 500, color: '#fff', background: '#0A0A0A', border: 'none', cursor: 'pointer' }}>
                  Speichern
                </button>
                <button onClick={() => setEditMode(false)}
                  className="px-5 py-2.5 rounded-lg" style={{ ...font, fontSize: '0.82rem', color: 'rgba(10,10,10,0.5)', background: '#fff', border: '1px solid rgba(10,10,10,0.1)', cursor: 'pointer' }}>
                  Abbrechen
                </button>
              </div>
            </div>
